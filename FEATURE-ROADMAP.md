...
| FR-004 | Deliberately migrate the first-party native Gallery source to the current GLAZE UI V1.3 / 1.3.0 Adaptive Resonance authority using exact source anchors and governed semantic roles. | High | Development — bounded repository-local V1.3 source mapping introduced on the current stacked candidate; whole-application acceptance remains incomplete. |
...
| FR-006 | Complete first-party photo-editor acceptance for crop, rotate, flip, Reset, non-destructive Save copy, process recreation, orientation/output fidelity, metadata/color behavior, failure/cancellation cases, accessibility, and representative devices. | High | Development — source/test/build foundations exist; representative acceptance remains incomplete. |
