...
grep -Fq 'GoreeCloud Gallery Stable Signing Runbook' docs/STABLE-SIGNING-RUNBOOK.md \
  || fail 'stable signing runbook does not identify its Gallery scope'
grep -Fq 'document the existence, purpose, owner, lifecycle, and validation of signing secrets without reproducing the active secret values' docs/STABLE-SIGNING-RUNBOOK.md \
  || fail 'stable signing runbook does not preserve secret-separation requirements'
grep -Fq 'GoreeCloud Gallery Real-Device Acceptance Runbook' docs/REAL-DEVICE-ACCEPTANCE-RUNBOOK.md \
  || fail 'real-device acceptance runbook does not identify its Gallery scope'
grep -Fq 'Glaze UI is mandatory for controlled Gallery surfaces' docs/REAL-DEVICE-ACCEPTANCE-RUNBOOK.md \
  || fail 'real-device runbook does not preserve the Glaze UI acceptance boundary'
