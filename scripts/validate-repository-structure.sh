...
for patch_line in gc1 gc2 gc3 gc4 gc5 gc6 gc7 gc8 gc9 gc10 gc11 gc12 gc13; do
...
grep -Fq 'Current Gallery implementation line: `gc.13`' docs/GLAZE-UI.md || fail 'gc.13 Glaze UI implementation line is not documented'
...
grep -Fq 'VERSION_CODE = "10013"' patches/gc13/build_goreecloud_gallery_gc13.py || fail 'gc.13 does not set the current test-build version code'
grep -Fq 'GLAZE_UI_VERSION = "1.0.0"' patches/gc13/build_goreecloud_gallery_gc13.py || fail 'gc.13 does not identify the Glaze UI target version'
